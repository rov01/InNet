...
var timerModule = angular.module('timer', [])
  .directive('timer', ['$compile', function ($compile) {
    return  {
      ...
      controller: ['$scope', '$element', '$attrs', '$timeout', 'I18nService', '$interpolate', 'progressBarService', 'SocketSvc', function ($scope, $element, $attrs, $timeout, I18nService, $interpolate, progressBarService, SocketSvc) {
        ...
        SocketSvc.on('timerStart',function(data){
          $scope.start();
        });

        $scope.$on('timer-start', function () {
          $scope.start();
          SocketSvc.emit('timerStart',{ start : 'timer-start'});
        });
        ...
        var tick = function tick() {
          ...
          if(typeTimer !== null){
            ...
            if ($scope.progressBar < 30 ) {
              $scope.progressState = "progress-bar-primary"; 
              SocketSvc.emit('stateUpdate',{
                id            : $scope.st._id,
                progress      : $scope.progressBar,
                progressState : $scope.progressState,
                millis        :  $scope.millis
              });
            
            } else if( 30 <= $scope.progressBar  && $scope.progressBar < 60 ) {
              $scope.progressState = "progress-bar-success";
              SocketSvc.emit('stateUpdate',{
                id : $scope.st._id,
                progress : $scope.progressBar,
                progressState :  $scope.progressState,
            ...
            } else if( 60 <= $scope.progressBar && $scope.progressBar < 90 ) {
              $scope.progressState = "progress-bar-warning";
              SocketSvc.emit('stateUpdate',{
                id : $scope.st._id,
                progress : $scope.progressBar,
                progressState :  $scope.progressState,
              ...
              $scope.progressState = "progress-bar-danger";
              SocketSvc.emit('stateUpdate',{
                id : $scope.st._id,
                progress : $scope.progressBar,
                progressState :  $scope.progressState,
